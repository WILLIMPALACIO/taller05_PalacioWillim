...
import { FIX_PELICULAS } from "../../mocks/Pelicula-mocks";
import { FIX_PELICULA_GENERO } from "../../utilidades/dominios/DomGenero";

export const PeliAdmin = () => {
  const [arrPeliculas] = useState<Pelicula[]>(FIX_PELICULAS);

  const obtenerNombreGenero = (valor: string) => {
    for (const objGen of FIX_PELICULA_GENERO) {
              ...
              {arrPeliculas.map((miPeli: Pelicula) => (
                <tr className="align-middle" key={miPeli.codPelicula}>
                  ...
                  <td className="text-center">
                    <a href="/eliminar/{miPeli.codPelicula}">
                      <i className="fa-solid fa-trash-can rojito"></i>
                    </a>
                    {" "}
                    <a href="/editar/{miPeli.codPelicula}">
